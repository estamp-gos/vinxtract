...
export const DOCUKIT_PACKAGES = {
  basic: { USD: 29, EUR: 24.99 },
  premium: { USD: 59, EUR: 47 },
  ultimate: { USD: 89, EUR: 89 },
};

/** Redirect after successful payment on DocuKit / Paddle */
export const DOCUKIT_SUCCESS_URL = 'https://www.vinxtract.com/thankyou';

/** All checkouts use the basic package per product requirements */
export const DOCUKIT_CHECKOUT_PACKAGE = 'basic';

/** Paddle price ID for €24.99 EUR checkout */
export const DOCUKIT_PADDLE_PRICE_ID = 'pri_01krk1p8r73ba0kv41bfy09k89';
...
export function getPackagePrice(pkg = DOCUKIT_CHECKOUT_PACKAGE, currency = 'EUR') {
  ...
  return DOCUKIT_PACKAGES[pkg]?.[priceKey] ?? DOCUKIT_PACKAGES.basic[priceKey];
}
...
export function buildDocuKitPaymentUrl(params) {
  ...
  const amount =
    ...
    DOCUKIT_PACKAGES.basic[priceKey];
