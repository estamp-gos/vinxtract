...
export const DOCUKIT_PACKAGES = {
  basic: { USD: 29, GBP: 23 },
  premium: { USD: 59, GBP: 47 },
  ultimate: { USD: 89, GBP: 71 },
};

/** All checkouts use the ultimate package per product requirements */
export const DOCUKIT_CHECKOUT_PACKAGE = 'ultimate';

/** @param {string} [pkg] @param {string} [currency] USD | GBP | EUR */
export function getPackagePrice(pkg = DOCUKIT_CHECKOUT_PACKAGE, currency = 'GBP') {
  const code = (currency || 'GBP').toUpperCase();
  const priceKey = code === 'GBP' ? 'GBP' : 'USD';
 ...
 * @param {string} [params.currency] - USD, GBP, EUR (default USD)
...
export function buildDocuKitPaymentUrl(params) {
  ...
  const priceKey = currency === 'GBP' ? 'GBP' : 'USD';
  const amount =
    params.amount ??
  ...
  if (params.success_url) query.set('success_url', params.success_url);
...
export function getDefaultSuccessUrl() {
  if (typeof window !== 'undefined' && window.location.hostname.includes('vinxtract.com')) {
    return 'https://www.vinxtract.com/thankyou';
  }
